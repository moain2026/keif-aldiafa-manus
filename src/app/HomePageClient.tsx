...
function MomentCard({ m, i }: { m: (typeof moments)[0]; i: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.08, duration: 0.5 }} className="relative rounded-2xl overflow-hidden group cursor-pointer" style={{ aspectRatio: i === 0 || i === 3 ? "3/4" : "4/3" }}>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        ...
        <motion.div className="mt-2 h-0.5 bg-[#B8860B] rounded-full" initial={{ width: 0 }} whileInView={{ width: "100%" }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.08 + 0.4, duration: 0.5 }} style={{ originX: 1 }} />
      </div>
    </motion.div>
  );
}
...
function TestimonialCard({ t, i }: { t: (typeof testimonials)[0]; i: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.12, duration: 0.5 }} className="p-6 rounded-2xl card-luxury relative">
    ...
    <div>
      ...
      <section className="py-8 px-4" aria-label="إحصائيات كيف الضيافة" style={{ background: "linear-gradient(90deg, rgba(184,134,11,0.04) 0%, rgba(184,134,11,0.08) 50%, rgba(184,134,11,0.04) 100%)", borderTop: "1px solid rgba(184,134,11,0.1)", borderBottom: "1px solid rgba(184,134,11,0.1)" }}>
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          {stats.map((s, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.1 }}><p className="gold-gradient-text" style={{ fontSize: "clamp(1.6rem, 4vw, 2.2rem)", fontWeight: 900,  }}>{s.num}</p><p className="text-[#F5F5DC]/55 text-sm mt-1">{s.label}</p></motion.div>))}
        </div>
      </section>
      ...
      <section className="py-24 px-4 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.7 }} className="relative">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: 0.4, duration: 0.5 }} className="absolute -bottom-5 -left-5 px-5 py-4 rounded-2xl" style={{ background: "linear-gradient(135deg, rgba(15,12,5,0.95), rgba(25,20,8,0.97))", border: "1px solid rgba(184,134,11,0.3)", boxShadow: "0 10px 40px rgba(0,0,0,0.5)" }}>
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.7, delay: 0.1 }}>
            <p className="text-[#B8860B] mb-3" style={{ fontSize: "0.75rem", letterSpacing: "0.35em" }}>✦ تجربتنا ✦</p>
            <h2 className="text-[#F5F5DC] mb-5 font-tajawal" style={{ fontSize: "clamp(1.5rem, 3.5vw, 2.2rem)", fontWeight: 800, lineHeight: 1.3 }}>نحن نصنع لحظات<br /><span className="gold-gradient-text">لا تُنسى</span></h2>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-10">
            {[{ img: WAITER_IMG, title: "الخدمات الرجالية", sub: "Male Hospitality" }, { img: HOTEL_IMG, title: "الخدمات الفنية", sub: "Artistic Services" }, { img: EQUIP_IMG, title: "المعدات الفاخرة", sub: "Luxury Equipment" }, { img: DATES_IMG, title: "التقديمات الراقية", sub: "Luxury Offerings" }].map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.08 }} className="relative rounded-2xl overflow-hidden group cursor-pointer" style={{ aspectRatio: "16/9" }}>
